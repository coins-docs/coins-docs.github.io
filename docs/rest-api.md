...
2025-12-19: Added [Java SDK](https://docs.coins.ph/rest-api/#connectors) support for major APIs, enabling developers to integrate key endpoints through standardized client methods.
...
data.status	| The order status. Possible values are: `TODO`, `PROCESSING`, `SUCCESS`, and `FAILED`. `TODO` means the order is ready to process (pending execution). `PROCESSING` means the server is processing the order. `SUCCESS` means the order has been completed successfully. `FAILED` means the order processing failed. Please check the `errorMessage` field in the Order List API response for the detailed failure reason.
...
status | STRING | No | Filter by order status. Possible values: `TODO`, `PROCESSING`, `SUCCESS`, `FAILED`. Please refer to [accept-quote API → order status] for the description of each status.
page | int    | No |
size | int    | No | Default:10, Max:200
type | STRING | NO | Specifies the trading mode. Please refer to [get-quote API  → type]
quoteId | STRING | No | Filter by quote ID. Please refer to [accept-quote API → quoteId].
orderId | STRING | No | Filter by order ID. Please refer to [accept-quote API → orderId].
...
Field name	| Description
----|---
orderId	| Order ID generated by the server. Please refer to [accept-quote API → orderId].
quoteId	| Order reference quote Id. Please refer to [accept-quote API → quoteId].
userId	| user id.
...
price	| price.
status	| Order status. Possible values: `TODO` (ready to process), `PROCESSING`, `SUCCESS`, `FAILED`. Please refer to [accept-quote API → order status] for the description of each status.
...
### OTC Trade endpoints (for Business Account)
The following endpoints are available to Business Account users only. To migrate to a Business Account, please contact our support team.
...
maxSourceAmount	| amount range max value.
precision	| The level of precision in decimal places used.
settlementType | The settlement type supported for this currency pair, namely ```INSTANT```, ```MANUAL_DEFER``` and ```AUTO_DEFER```
expiry | Validity of the quote in seconds.
deferredSettlementExpiry | The time of expiry for deferred settlement quotes for this currency pair in seconds.
dailySettlementLimit | Accumulated daily settlement limit by user ID, in USDT.
...
sourceAmount | STRING | NO        |The amount of sourceCurrency. You only need to fill in either the source amount or the target amount. If both are filled, it will result in an error.
targetAmount | STRING | NO        |The amount of targetCurrency. You only need to fill in either the source amount or the target amount. If both are filled, it will result in an error.
settlementType         | STRING | YES        | The settlement type the user would like to choose, namely ```INSTANT```, ```MANUAL_DEFER``` and ```AUTO_DEFER```
...
#### Accept RFQ (OTC TRADE for Business Account)
...
status	| Status of the order: ```SUCCEEDED``` / ```PROCESSING```
deferredSettlementExpiryUntil	| Unix timestamp indicating when the deferred settlement order will no longer be valid. Optional, only when settlementType =  ```MANUAL_DEFER``` or ```AUTO_DEFER```
...
```javascript
{
  ...
  "data": {
         "orderId" : "49d10b74c60a475298c6bbed08dd58fa",
         "status": "SUCCESS",
         "deferredSettlementExpiryUntil": "1747306565330"
  },
  "error": "ok"
}
```
...
#### Manual settle (OTC TRADE for Business Account)
```shell
POST /openapi/otc-trade/v1/manual-settle
```
This API is only for manually settling deferred-settlement orders. By calling this API, the user triggers manual settlement.
...
Field name	| Description
----|---
status	| Status of the order: ```SUCCEEDED``` / ```FAILED```
...
#### Get order (OTC TRADE for Business Account)

```shell
POST /openapi/otc-trade/v1/query-order-history
```
This endpoint retrieves order history with the option to define a specific time period using start and end times, limited to the last 180 days.
...
targetAmount	| target currency amount.
price	| price.
fee | The amount charged on the order by the partner to the customer.
status	| Order status: <br>&bull; `OPEN` (The order has been placed but not yet matched) <br>&bull;  `PENDING-SETTLEMENT` (The order has been matched but not yet settled) <br>&bull;  `EXPIRED` (The order has been matched but expired before settlement)<br>&bull;  `SETTLED` (The order has been settled)<br>&bull;  `FAILED` (The order is failed)
createdAt	| Order create time.
settlementType	| The settlement type the user would like to choose, namely INSTANT, MANUAL_DEFER and AUTO_DEFER
...
```javascript
{
  "status": 0,
   ...
   "data": [
  {
          "orderId": "21770585969155624968otc-trade",
          "quoteId": "21770585969155624968otc-trade",
          "sourceCurrency": "USDT",
          "targetCurrency": "BTC",
          "sourceAmount": "100",
          "targetAmount": "0.00145855",
          "price": "68561.24232972",
          "fee": "0",
          "status": "FAILED",
          "createdAt": "1774261614000",
          "errorCode": "",
          "errorMessage": "",
          "settlementType": "AUTO_DEFER"
      },
      {
          "orderId": "21755084215605749768otc-trade",
          "quoteId": "21755084215605749768otc-trade",
          "sourceCurrency": "USDT",
          "targetCurrency": "BTC",
          "sourceAmount": "77",
          "targetAmount": "0.00107892",
          "price": "71367.66396026",
          "fee": "0",
          "status": "SETTLED",
          "createdAt": "1774076814000",
          "errorCode": "",
          "errorMessage": "",
          "settlementType": "AUTO_DEFER"
      }
